...
namespace BinarySearchNamespace
{
    public static class BinarySearch 
    {
        ...
        /// <param name="array">array in which needed to find item</param>
        /// <param name="item">needed to find</param>
        /// <param name="comparer">interface for compare to items</param>
        /// <returns>index of number or -1 if isnt exist</returns>
        /// <exception cref="ArgumentNullException">if array is null</exception>
        /// <exception cref="ArgumentNullException">if comparer is null</exception>
        public static int StartBinarySearch<T>(this T[] array, T item, IComparer<T> comparer)
        {
            if (array == null)
            {
                throw new ArgumentNullException(nameof(array));
            ...
            return array.StartBinarySearch(item, comparer.Compare);
        }
        ...
        /// <exception cref="ArgumentNullException">if array is null</exception>
        /// <exception cref="ArgumentNullException">if comparer is null</exception>
        /// <exception cref="InvalidOperationException">if isnt ComparerT and isnt delegate</exception>
        public static int StartBinarySearch<T>(this T[] array, T item)
        {
            if (array == null)
            {
                throw new ArgumentNullException(nameof(array));
            ...
            {
                return array.StartBinarySearch(item, Comparer<T>.Default.Compare);
            }

            throw new InvalidOperationException($"{item.GetType()} isnt implemented ICompare");
        }
